$(document).ready(function () {
  ...
  async function fetchTotalDataCount() {
    return new Promise((resolve, reject) => {
      $.ajax({
        type: "POST",
        url: "../user.php",
        data: { action: "getTableCount", table: "Contact_Us" },
        success: function (response) {
          var parsedResponse = JSON.parse(response);
          if (parsedResponse && parsedResponse.status) {
  ...
  async function loadData(query = "") {
    ...
    try {
      ...
      if (totalDataCount && data) {
        ...
        $tableBody.html(
          '<tr><td colspan="100%"><h3 class="text-center">Contacts not found</h3></td></tr>'
        );
      }
    } catch (error) {
  ...
  function renderTable(page) {
    ...
    $.each(filteredData, function (index, item) {
      ...
      $("<td>").html(`<textarea id="w3review" name="w3review" rows="5" cols="30">${item.message}</textarea>`).appendTo($row);
